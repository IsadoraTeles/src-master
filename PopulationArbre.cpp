...
Population_Arbre::Population_Arbre()
{
	generations = 0;
}

void Population_Arbre::setup(float _mutationRate, int _numArbres)
{
	float w = 512;
	float h = 512;
	...
	populationArbre.resize(numArbres);
...
void Population_Arbre::drawPath()
{
	// Run every plant
	for (auto &p : populationArbre)
	{
...
void  Population_Arbre::evolvePath()
{
	// Run every plant
	for (auto &p : populationArbre)
	{
...
void Population_Arbre::calcFitness(float &_pluieValue, float &_ventValue, float &_soleilValue, float &_humiditeValue)
{
...
void Population_Arbre::selection()
{
	// clear the vector
	matingPool.clear();
...
void Population_Arbre::reproduction()
{
	// Refill the population with children from the mating pool
	for (int i = 0; i < populationArbre.size(); i++)
	{
		...
		float z = 0.0;
		ofVec3f origin(x, y, z);
		populationArbre[i] = new Phenotype_Arbre();
		populationArbre[i]->setup(child, x, y);
		populationArbre[i]->calculate();
	}
	generations++;
}

float Population_Arbre::getMaxFitness()
{
	float record = 0;
	for (auto &p : populationArbre)
	{
