...
Population_Herbe2::Population_Herbe2()
{
	generations = 0;
}

void Population_Herbe2::setup(float _mutationRate, int _numHerbes)
{
	float w = 512;
	float h = 512;
	...
	populationHerbe2.resize(numHerbes);
...
void Population_Herbe2::calcFitness(float &_pluieValue, float &_ventValue, float &_soleilValue, float &_humiditeValue)
{
...
void Population_Herbe2::selection()
{
	// clear the vector
	matingPool.clear();
...
void Population_Herbe2::reproduction()
{
	// Refill the population with children from the mating pool
	for (int i = 0; i < populationHerbe2.size(); i++)
	{
		...
		float z = 0.0;
		ofVec3f origin(x, y, z);
		populationHerbe2[i] = new Phenotype_Herbe2();
		populationHerbe2[i]->setup(child, x, y);
		populationHerbe2[i]->calculate();
	}
	generations++;
}

float Population_Herbe2::getMaxFitness()
{
	float record = 0;
	for (auto &p : populationHerbe2)
	{
